...
func TestStateProposerSelection0(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), true, vss[1:]...)
...
func TestStateProposerSelection2(t *testing.T) {
	...
	for i := int32(0); int(i) < len(vss); i++ {
		...
		rs := cs1.GetRoundState()
		signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, nil, rs.ProposalBlockParts.Header(), true, vss[1:]...)
...
func TestStateBadProposal(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlock.Hash(), bps.Header(), false, vs2)
	ensurePrevote(voteCh, height, round)
	...
	require.NoError(t, err)
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, propBlock.Hash(), bps2.Header(), true, vs2)
}
...
func TestStateFullRound2(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlockHash, propPartSetHeader, false, vs2)
	ensurePrevote(voteCh, height, round) // prevote
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, propBlockHash, propPartSetHeader, true, vs2)
	ensurePrecommit(voteCh, height, round)
...
func TestStateLockNoPOL(t *testing.T) {
	...
	*/
	...
	// prevote arrives from vs2:
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, theBlockHash, thePartSetHeader, false, vs2)
	ensurePrevote(voteCh, height, round) // prevote
	...
	hash[0] = (hash[0] + 1) % 255
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, hash, thePartSetHeader, true, vs2)
	ensurePrecommit(voteCh, height, round) // precommit
	...
	*/
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, hash, bps.Header(), false, vs2)
	ensurePrevote(voteCh, height, round)
	...
	require.NoError(t, err)
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, hash, bps2.Header(), true, vs2)
	ensurePrecommit(voteCh, height, round)
	...
	*/
	...
	require.NoError(t, err)
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, hash, bps0.Header(), false, vs2)
	ensurePrevote(voteCh, height, round)
	...
	*/
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlock.Hash(), bps4.Header(), false, vs2)
	ensurePrevote(voteCh, height, round)
...
func TestStateLockPOLRelock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, theBlockHash, theBlockParts, false, vs2, vs3, vs4)
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, nil, types.PartSetHeader{}, true, vs2, vs3, vs4)
	...
	*/
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlockHash, propBlockParts.Header(), false, vs2, vs3, vs4)
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, propBlockHash, propBlockParts.Header(), true, vs2, vs3)
...
func TestStateLockPOLUnlock(t *testing.T) {
	...
	*/
	...
	// add precommits from the rest
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, nil, types.PartSetHeader{}, true, vs2, vs4)
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, theBlockHash, theBlockParts, true, vs3)
	...
	*/
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, nil, types.PartSetHeader{}, false, vs2, vs3, vs4)
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, nil, types.PartSetHeader{}, true, vs2, vs3)
...
func TestStateLockPOLUnlockOnUnknownBlock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, firstBlockHash, firstBlockParts, false, vs2, vs3, vs4)
	...
	*/
	...
	// now lets add prevotes from everyone else for the new block
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, secondBlockHash, secondBlockParts.Header(), false, vs2, vs3, vs4)
	...
	*/
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, thirdPropBlockHash, thirdPropBlockParts.Header(), false, vs2, vs3, vs4)
...
func TestStateLockPOLSafety1(t *testing.T) {
	...
	prevotes := signVotes(types.SignedMsgType_PREVOTE, propBlock.Hash(), bps.Header(), false, vs2, vs3, vs4)
...
func TestStateLockPOLSafety2(t *testing.T) {
	...
	// the others sign a polka but we don't see it
	prevotes := signVotes(types.SignedMsgType_PREVOTE, propBlockHash0, propBlockParts0.Header(), false, vs2, vs3, vs4)
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlockHash1, propBlockParts1.Header(), false, vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, nil, types.PartSetHeader{}, true, vs2, vs4)
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, propBlockHash1, propBlockParts1.Header(), true, vs3)
...
func TestProposeValidBlock(t *testing.T) {
	...
	bps, err := propBlock.MakePartSet(partSize)
	require.NoError(t, err)
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlockHash, bps.Header(), false, vs2, vs3, vs4)
...
func TestSetValidBlockOnDelayedPrevote(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlockHash, propBlockParts.Header(), false, vs2)

	// vs3 send prevote nil
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, nil, types.PartSetHeader{}, false, vs3)
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlockHash, propBlockParts.Header(), false, vs4)
...
func TestExtendVoteCalledWhenEnabled(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			signAddVotes(cs1, types.SignedMsgType_PREVOTE, blockID.Hash, blockID.PartSetHeader, false, vss[1:]...)
			...
			signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, blockID.Hash, blockID.PartSetHeader, testCase.enabled, vss[1:]...)
			ensureNewRound(newRoundCh, height+1, 0)
...
func TestVerifyVoteExtensionNotCalledOnAbsentPrecommit(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, blockID.Hash, blockID.PartSetHeader, false, vss...)
	ensurePrevoteMatch(t, voteCh, height, round, blockID.Hash)
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, blockID.Hash, blockID.PartSetHeader, true, vss[2:]...)
	ensureNewRound(newRoundCh, height+1, 0)
	m.AssertExpectations(t)
...
func TestPrepareProposalReceivesVoteExtensions(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, blockID2.Hash, blockID2.PartSetHeader, true, vss[1:]...)
	ensureNewRound(newRoundCh, height, round)
	ensureNewProposal(proposalCh, height, round)
...
func TestFinalizeBlockCalled(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, blockID.Hash, blockID.PartSetHeader, true, vss[1:]...)
			ensurePrecommit(voteCh, height, round)
...
func TestVoteExtensionEnableHeight(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			signAddVotes(cs1, types.SignedMsgType_PREVOTE, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), false, vss[1:]...)
			ensurePrevoteMatch(t, voteCh, height, round, rs.ProposalBlock.Hash())
			...
			for _, vs := range vss[1:] {
				vote, err := vs.signVote(types.SignedMsgType_PRECOMMIT, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), ext, testCase.hasExtension)
				require.NoError(t, err)
				addVotes(cs1, vote)
			}
...
func TestEmitNewValidBlockEventOnCommitWithoutBlock(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, propBlockHash, propBlockParts.Header(), true, vs2, vs3, vs4)
	ensureNewValidBlock(validBlockCh, height, round)
...
func TestStartNextHeightCorrectlyAfterTimeout(t *testing.T) {
	...
	// add precommits
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, nil, types.PartSetHeader{}, true, vs2)
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, theBlockHash, theBlockParts, true, vs3)
	...
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, theBlockHash, theBlockParts, true, vs4)
...
func TestStateSlashingPrevotes(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, hash, rs.ProposalBlockParts.Header(), vs2)
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), vs2)
...
func TestStateSlashingPrecommits(t *testing.T) {
	...
	hash := rs.ProposalBlock.Hash()
	hash[0] = byte(hash[0]+1) % 255
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, hash, rs.ProposalBlockParts.Header(), vs2)

	// NOTE: we have to send the vote for different block first so we don't just go into precommit round right
	// away and ignore more prevotes (and thus fail to slash!)

	// add precommit from vs2
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), vs2)
...
func TestStateHalt1(t *testing.T) {
	...
	signAddVotes(cs1, types.SignedMsgType_PREVOTE, propBlock.Hash(), propBlockParts.Header(), false, vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, nil, types.PartSetHeader{}, true, vs2) // didnt receive proposal
	signAddVotes(cs1, types.SignedMsgType_PRECOMMIT, propBlock.Hash(), propBlockParts.Header(), true, vs3)
	// we receive this later, but vs3 might receive it earlier and with ours will go to commit!
	precommit4 := signVote(vs4, types.SignedMsgType_PRECOMMIT, propBlock.Hash(), propBlockParts.Header(), true)
...
func TestStateOutputVoteStats(t *testing.T) {
	...
	vote := signVote(vss[1], types.SignedMsgType_PRECOMMIT, randBytes, types.PartSetHeader{}, true)
...
func signAddPrecommitWithExtension(
	...
	extension []byte,
	stub *validatorStub) {
	v, err := stub.signVote(types.SignedMsgType_PRECOMMIT, hash, header, extension, true)
