...
func TestValidatorSet_VerifyCommit_All(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.description, func(t *testing.T) {
			_, valSet, vals := randVoteSet(tc.height, round, SignedMsgType_PRECOMMIT, tc.valSize, 10, false)
...
func TestValidatorSet_VerifyCommit_CheckAllSignatures(t *testing.T) {
	...
	voteSet, valSet, vals := randVoteSet(h, 0, SignedMsgType_PRECOMMIT, 4, 10, false)
...
func TestValidatorSet_VerifyCommitLightTrusting(t *testing.T) {
	var (
		blockID                       = makeBlockIDRandom()
		voteSet, originalValset, vals = randVoteSet(1, 1, SignedMsgType_PRECOMMIT, 6, 1, false)
...
func TestValidatorSet_VerifyCommitLightTrustingErrorsOnOverflow(t *testing.T) {
	var (
		blockID               = makeBlockIDRandom()
		voteSet, valSet, vals = randVoteSet(1, 1, SignedMsgType_PRECOMMIT, 1, MaxTotalVotingPower, false)
		extCommit, err        = MakeExtCommit(blockID, 1, 1, voteSet, vals, time.Now(), false)
	)
	require.NoError(t, err)
